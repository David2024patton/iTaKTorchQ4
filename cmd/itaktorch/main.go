...
const defaultCacheDir = "" // empty = let puller use os.UserHomeDir()/.itaktorch/models/
...
func printUsage() {
	fmt.Println(`iTaKTorch - Go-Native LLM Inference Engine

Usage:
  itaktorch <command> [options]
...
Examples:
  itaktorch serve --model ./models/qwen3-0.6b.gguf --port 11434
  itaktorch serve --models-dir ~/.itaktorch/models --max-models 2 --port 11434
  itaktorch serve --model main.gguf --draft-model draft.gguf --speculative-tokens 5
...
func cmdServe(args []string) {
	...
	if *useMock {
		...
		if cacheDir == "" {
			if home, err := os.UserHomeDir(); err == nil {
				cacheDir = home + "/.itaktorch/models/"
			} else {
				cacheDir = "./models"
			}
		}
		...
		*modelPath = convertedPath // Transparently handoff the new .gguf path
		...
		fmt.Fprintf(os.Stderr, "Usage: itaktorch serve --model <path.gguf> --port <port>\n")
		fmt.Fprintf(os.Stderr, "       itaktorch serve --models-dir <dir> --max-models 2 --port <port>\n")
		fmt.Fprintf(os.Stderr, "       itaktorch serve --mock --port <port>\n")
		os.Exit(1)
	}
	...
	go func() {
		...
		if pgoFile != nil {
			...
			fmt.Println("[iTaK Torch] PGO: rebuild with: go build -pgo=" + *pgoCapture + " ./cmd/itaktorch/")
		}
...
func cmdModels() {
	...
	if len(models) == 0 {
		...
		fmt.Println("Run 'itaktorch catalog' to see available models.")
		return
	}
...
func cmdCatalog() {
	...
	fmt.Println("iTaKTorch Model Catalog:")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("Pull a model: itaktorch pull <name>")
	fmt.Println("See what fits your hardware: itaktorch recommend")
}
...
func cmdRecommend() {
	...
	fmt.Println("iTaKTorch Hardware Detection")
	fmt.Println("=========================")
	...
	if len(pairs) > 0 {
		...
		fmt.Println()
		fmt.Println("  Usage: itaktorch serve --model <main.gguf> --draft-model <draft.gguf> --speculative-tokens 5")
	}
...
func cmdPull(args []string) {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Usage: itaktorch pull <model-name>\n")
		fmt.Fprintf(os.Stderr, "Run 'itaktorch catalog' to see available models.\n")
		os.Exit(1)
	}
...
func cmdOllamaPull(args []string) {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Usage: itaktorch ollama-pull <model:tag>\n")
		fmt.Fprintf(os.Stderr, "Example: itaktorch ollama-pull nemotron-3-nano:30b-a3b-q4_K_M\n")
		os.Exit(1)
	}
...
func cmdHFPull(args []string) {
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Usage: itaktorch hf-pull <repo> <filename>\n")
		fmt.Fprintf(os.Stderr, "Example: itaktorch hf-pull Qwen/Qwen3-0.6B-GGUF qwen3-0.6b-q4_k_m.gguf\n")
		os.Exit(1)
	}
	...
	if filename == "" {
		...
		fmt.Println()
		fmt.Println("Pull a file: itaktorch hf-pull", repo, "<filename>")
		return
	}
...
// Usage:
//
//	itaktorch bench <model.gguf> [flags]
//
// Flags:
//
...
func cmdBench(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "Usage: itaktorch bench <model.gguf> [--backend gotensor|ffi] [--json] [--iterations N] [--gpu-layers N]")
		os.Exit(1)
	}
...
// Usage: itaktorch chat <model.gguf> [--bench] [--sparse N]
func cmdChat(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "Usage: itaktorch chat <path-to-gguf> [--bench] [--sparse 70] [--gpu]")
		os.Exit(1)
	}
